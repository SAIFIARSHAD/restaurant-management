...
export const recalculateOrderStatus = (items: IOrderItem[]): OrderStatus => {
  ...
  const anyAccepted = activeItems.some((item) => item.status === 'accepted');
...
export const updateOrderStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (status === 'preparing' && previousStatus !== 'preparing') {
      for (const item of existingOrder.items as any[]) {
        ...
        for (const ingredient of recipe.ingredients) {
          ...
          console.log(
            `Stock deducted: ${rawMaterial.name} → -${deductAmount} ${rawMaterial.unit}`
          );
          ...
          await RawMaterialLog.create({
            ...
            reason: `Order #${existingOrder.orderNumber} — ${item.name} x${item.quantity}`,
    ...
    if (status === 'ready') {
      emitOrderReady(io, restaurantId, payload);
      io.to(`restaurant_${restaurantId}`).emit('order_ready', payload);
      io.to(`kds_${restaurantId}`).emit('order_status_updated', payload);
    }

    if (status === 'cancelled') {
      emitOrderCancelled(io, restaurantId, payload);
      io.to(`kds_${restaurantId}`).emit('order_cancelled', payload);
    }
