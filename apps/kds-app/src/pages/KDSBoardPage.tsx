...
function getRangeForFilter(dateFilter: DateFilterKey, customFrom: string, customTo: string): { from?: string; to?: string } {
  const today = new Date();
  const startOfToday = new Date(today.getFullYear(), today.getMonth(), today.getDate());
  const endOfToday = new Date(startOfToday);
  endOfToday.setHours(23, 59, 59, 999);

  if (dateFilter === 'today') return { from: startOfToday.toISOString(), to: endOfToday.toISOString() };
...
export default function KDSBoardPage() {
  ...
  const {
    user, selectedStation, setUser, setSelectedStation,
    orders, completedOrders, setOrders, setCompletedOrders,
    isConnected, updateOrderStatus, updateItemStatus,
  } = useKDSStore();
  ...
  useEffect(() => {
    ...
    if (!session?.station) { navigate('/station-select', { replace: true }); return; }
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (!validSession) { navigate('/login', { replace: true }); return; }
      setNow(new Date());
    }, 1000);
    ...
    const loadOrders = async () => {
      try {
        ...
        const [active, completed] = await Promise.all([
          getKDSOrders({ station: currentStation.stationType, from, to }),
          getCompletedOrders({ station: currentStation.stationType, from, to }),
        ]);
        setOrders(active);
  ...
  useEffect(() => {
    ...
    const latest = [...orders, ...completedOrders].find((o) => o._id === selectedOrder._id) || null;
    if (latest) setSelectedOrder(latest);
  }, [orders, completedOrders, selectedOrder]);

  const allOrders = useMemo(() => {
    return [...orders, ...completedOrders].sort(
  ...
  const handleAdvanceItem = async (orderId: string, item: IOrderItem) => {
    const nextMap: Record<string, string> = { pending: 'accepted', accepted: 'preparing', preparing: 'ready', ready: 'served' };
    const nextStatus = nextMap[item.status];
    if (!nextStatus) return;

    try {
      setActionLoading(item._id);
      const res = await updateItemStatusApi(orderId, item._id, nextStatus as IOrderItem['status']);
      updateItemStatus(orderId, item._id, nextStatus as IOrderItem['status'], res.orderStatus);
    } catch (error) {
  ...
  const handleChangeStation = () => {
    ...
    localStorage.setItem('kds_session', JSON.stringify({ ...sessionData, station: null }));
          ...
          <div className="px-4 md:px-6 py-2.5">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h1 className="text-base md:text-lg font-bold tracking-tight text-white">ZaikaFlow KDS</h1>
              ...
              <div className="flex flex-none items-center gap-2">
                ...
                <div className={`hidden sm:flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-xs font-medium ${
                  isConnected ? 'border-green-500/25 bg-green-500/10 text-green-400' : 'border-red-500/25 bg-red-500/10 text-red-400'
                }`}>
                  <span className={`h-1.5 w-1.5 rounded-full ${isConnected ? 'bg-green-400 animate-pulse' : 'bg-red-400'}`} />
                  {isConnected ? 'Live' : 'Offline'}
                </div>

                <button onClick={handleChangeStation} className="rounded-full border border-orange-500/25 bg-orange-500/10 px-3 py-1.5 text-xs font-medium text-orange-400 transition hover:bg-orange-500/20">
                  Station
                </button>

                <button onClick={handleLogout} className="rounded-full border border-zinc-700 bg-zinc-800 px-3 py-1.5 text-xs font-medium text-zinc-300 transition hover:bg-zinc-700">
                  Logout
                </button>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-4 md:px-6 pt-4 space-y-3">
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-3.5">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">Order Status</p>
                ...
                {tabs.map((tab) => (
                  <button
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all ${
                      activeTab === tab.key ? 'bg-orange-500 text-white shadow-md' : 'bg-zinc-800 text-white/80 hover:bg-zinc-700 border border-zinc-700/50'
                    }`}
                  >
                    {tab.label} ({tab.count})
                  </button>
            ...
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-3.5">
              <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">Date Range</p>
              <div className="flex flex-wrap items-center gap-2">
                {([
                  ['today', 'Today'], ['yesterday', 'Yesterday'],
                  ['last3Days', 'Last 3 Days'], ['last7Days', 'Last 7 Days'], ['custom', 'Custom'],
                ] as [DateFilterKey, string][]).map(([key, label]) => (
                  <button
                    key={key}
                    onClick={() => {
                      setDateFilter(key);
                    ...
                    className={`px-4 py-1.5 rounded-full text-sm font-medium transition-colors ${
                      dateFilter === key ? 'bg-orange-500 text-white' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'
                    }`}
                  >
                    {label}
                  </button>
                ...
                {dateFilter === 'custom' && (
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <input type="date" value={customTo} min={customFrom || undefined}
                      ...
                      className="bg-zinc-800 border border-zinc-700 text-white text-sm rounded-lg px-3 py-1.5 outline-none" />
                    {(!customFrom || !customTo) && (
                      <button onClick={() => { const t = new Date(); setCustomFrom(toDateInputValue(t)); setCustomTo(toDateInputValue(t)); }}
          ...
          <main className="px-4 md:px-6 py-6">
            ...
            {loading ? (
              ...
              <div className="text-center py-16">
                ...
                <p className="text-zinc-600 text-sm mt-1">Try changing the status or date filter</p>
              </div>
            ) : (
              <div className="space-y-2">
