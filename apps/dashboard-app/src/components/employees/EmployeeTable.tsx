...
export default function EmployeeTable({ employees }: Props) {
  ...
  const getTemplateName = (templateId?: string) => {
    ...
    return shiftTemplates.find(t => t._id === templateId)?.name ?? null;
  };
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-zinc-800">
              <th className="px-4 py-3 text-left text-xs text-zinc-500 uppercase font-semibold tracking-wider">
                Employee
              </th>
              <th className="px-4 py-3 text-left text-xs text-zinc-500 uppercase font-semibold tracking-wider">
                Role
          ...
          <tbody>
            {employees.map(emp => {
              const templateName = getTemplateName(emp.shiftTemplateId);
              return (
                <tr
                  key={emp._id}
